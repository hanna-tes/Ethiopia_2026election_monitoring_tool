...
def generate_network_graph_from_groups(coordination_groups, top_n=50, layout='spring'):
    ...
    Properly identify source (originator) vs amplifiers using
    timestamps AND retweet patterns.
    """
    import networkx as nx
    import re
    import math
    ...
    for group in coordination_groups:
        ...
        for post in sample_posts:
            ...
            if rt_match:
                target = rt_match.group(1)
                if username not in account_rt_targets:
                    account_rt_targets[username] = set()
                account_rt_targets[username].add(target)
                account_is_rt[username] = True
            else:
        ...
        if original_posters:
            ...
            candidates_with_ts = [
                (acc, account_timestamps.get(acc, '9999-99-99'))
                for acc in original_posters
            ]
...
class LexiconsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
                ...
                for post in scan_pool:
                    ...
                    if matched_terms:
                        posts_with_terms.append({
                            'id':            post.id,
                            'text':          text,  # Kept full context available for tooltips or templates
        ...
        if selected_category and category_terms:
            ...
            for term, count in term_counts.most_common(15):
                ...
                for cat, terms in CONFIG.get('lexicon', {}).items():
                    if term in terms: metadata = terms[term]; break
                if not metadata:
        ...
        if not selected_category:
            try: cache.set(cache_key, shared, 7200)
            except Exception: pass
