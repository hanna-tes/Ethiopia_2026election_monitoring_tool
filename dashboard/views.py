    ...
    def post(self, request):
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                #  Parse timestamps and convert pd.NaT to None for Django
                if 'timestamp_share' in processed_df.columns:
                    # Coerce to datetime, invalid parsing becomes NaT
                    processed_df['timestamp_share'] = pd.to_datetime(processed_df['timestamp_share'], errors='coerce')
                    # Replace NaT with None so Django's DateTimeField accepts it safely
                    processed_df['timestamp_share'] = processed_df['timestamp_share'].where(pd.notnull(processed_df['timestamp_share']), None)
                ...
                for idx, row in processed_df.iterrows():
                    try:
                        ...
                        # Safely handle timestamp_share (ensure it's not pd.NaT)
                        ts_val = row.get('timestamp_share')
                        if pd.isna(ts_val) or ts_val is pd.NaT:
                            ts_val = None
                        ...
                        ProcessedPost.objects.create(
                            ...
                            timestamp_share=ts_val,  # Now guaranteed to be valid datetime or None
                            source_dataset=source_obj,
